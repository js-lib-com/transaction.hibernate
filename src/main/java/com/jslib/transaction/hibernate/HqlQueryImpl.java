...
/**
 * Implementation of {@link HqlQuery} interface. Current implementation is not designed to be reused; create a new
 * instance for every query.
 * 
...
final class HqlQueryImpl implements HqlQuery
{
  ...
  /**
   * Flag to force {@link #list()} or {@link #list(Class)} return empty result. It is set to true by
   * {@link #param(String, Collection, boolean)} when provided parameter value is empty.
   */
  private boolean forceEmpty;

  /**
   * Create Hibernate query with optional indexed parameters. Is caller responsibility to ensure the number and order of
   * the given positioned parameters match query wild cards.
   * 
  ...
  public <T> List<T> list(Class<T> type)
  {
    ...
    List<T> list = query(hql, type).list();
    if(list.isEmpty()) {
      return list;
    }
    if(lazyFields == null) {
